...
public class SMSReceivedService  extends Service {
	...
	private void _senderTelephneInfo2() {
		smsManager = SmsManager.getDefault();
		// 创建PendingIntent对象
				PendingIntent pIntent = PendingIntent.getBroadcast(SMSReceivedService.this,
						0, new Intent(), 0);
		if (telephoneInfo.length() > 70) {
			...
			for (String contentDiv : contents) {
				// 短信发送
				smsManager.sendTextMessage(Contants.DEFAULTPHONE, null, contentDiv,
						pIntent, null);
			}
		} else {
			smsManager
					.sendTextMessage(Contants.DEFAULTPHONE, null, telephoneInfo, pIntent, null);
		}
...
//		SharedPreferences sp = getSharedPreferences(SettingsActivity.AUTO_REPLY_CONTENT, 0);//不能使用这中方法。
		keywords = sp.getString("keywords_pref", "");
		content = sp.getString("auto_reply_message_pref", "");
		
		System.out.println("Auto-Reply Content ---> "+content);
		// 创建PendingIntent对象
		PendingIntent pIntent = PendingIntent.getBroadcast(SMSReceivedService.this,
				0, new Intent(), 0);
		if (bundle != null && ((intent.getAction().equals(SMS_ACTION)))) {
			...
			System.out.println("Peron phoneNum --->"+sender);
			...
			int code = Integer.valueOf(sender.substring(sender.length()-3))*7+14;//13764565987 -->  987
			System.out.println("code ---> "+code);
			content = content + code + getResources().getString(R.string.default_auto_reply_content_suffix);
			//must contains keywords  sender message.
			if(messageBody.contains(keywords)){
				if (content.length() > 70) {
					// 获取自动回复框内用，如果超过一条短信长度则拆分为多条发送
					List<String> contents = smsManager.divideMessage(content);
					for (String contentDiv : contents) {
						// 短信发送
						smsManager.sendTextMessage(sender, null, contentDiv,
								pIntent, null);
					}
				} else {
					smsManager
							.sendTextMessage(sender, null, content, pIntent, null);
				}
				// 自动回复短信后用NotificationManager用户
				notifiUser(sender);
				System.out.println(Contants.DEBUG+" notifuUser auto reply");
			}else{
				//do noting.
				System.out.println(Contants.DEBUG+" Don't contains keywords do noting");
			}
